...
def main() -> int:
    ...
    _write_json(
        evidence_dir / "metadata-index-before.json",
        {"sha256": metadata_before_sha, "size": metadata_size},
    )

    copy_result = create_verified_copy(
    ...
    memory = None
    inventory = None
    records: tuple[LegacyExpectedTablesRecord, ...] = ()
    try:
        ...
        interim_metadata_sha = _sha256_file(metadata_index)
        interim_facts = RecoverySourceFacts(
            formal_source_sha256=formal_before.content_sha256,
            verified_backup_sha256=backup_before.content_sha256,
            audit_copy_sha256=audit_before.content_sha256,
            audit_inventory_sha256=inventory.inventory_sha256,
            metadata_index_sha256_before=metadata_before_sha,
            metadata_index_sha256_after=interim_metadata_sha,
            store_count=inventory.store_count_after,
        ...
        calibration_passed, _, _ = evaluate_calibration_gate(
            records, calibration_analyses, interim_facts
        )
        recovery_analyses = ()
    ...
    shutil.rmtree(audit_copy)

    metadata_after_sha = _sha256_file(metadata_index)
    _write_json(
        evidence_dir / "metadata-index-after.json",
        {"sha256": metadata_after_sha, "size": metadata_index.stat().st_size},
    )
    ...
    facts = RecoverySourceFacts(
        formal_source_sha256=formal_after.content_sha256,
        verified_backup_sha256=backup_after.content_sha256,
        audit_copy_sha256=audit_before.content_sha256,
        audit_inventory_sha256=inventory.inventory_sha256,
        metadata_index_sha256_before=metadata_before_sha,
    ...
    batch_validator_sha = _sha256_file(
        ROOT / "training" / "sop" / "batch_validator.py"
    )
    ...
    result = build_expected_tables_recovery_proposal(
        records, analyses, facts, environment
    )
    ...
    return 0 if result.proposal_ready and not audit_copy.exists() else 1
