...
import tools.f5_level2_batch05_delivery as previous
...
BATCH_FILE = PROJECT_ROOT / "training" / "f5_level2_batch06.json"
SOURCE_EVIDENCE = Path(
    r"E:\3\_training_backups\f5-batch06-control-zone-scope-20260716-123023\evidence"
)
...
EXPECTED_FORMAL_SHA256 = "32324dd26bb48bb14e5cf376caa05131d6afe028f12783be4a17d48902ba57dc"
EXPECTED_CANDIDATE_ID = "D4_L2_WST_CONTROL_ZONE_003"
EXPECTED_SAMPLE_ID = "F5_L2_B06_SQL_001"
EXPECTED_TRAINING_LEVEL = "level2_sql_examples"
EXPECTED_TABLES = ["wst_control_zone"]
EXPECTED_COLUMNS = ["zone_code", "zone_name", "divide_basis"]
EXPECTED_QUESTION = "查询水安全溯源分区的分区编码、分区名称和划分依据，最多返回50条"
EXPECTED_SQL = """SELECT zone_code,
       zone_name,
       divide_basis
FROM wst_control_zone
LIMIT 50"""
...
BATCH04_SQL = previous.previous.EXPECTED_SQL
BATCH04_SAMPLE_ID = "F5_L2_B04_SQL_001"
BATCH05_QUESTION = previous.EXPECTED_QUESTION
BATCH05_TABLES = previous.EXPECTED_TABLES
BATCH05_COLUMNS = previous.EXPECTED_COLUMNS
BATCH05_SAMPLE_ID = "F5_L2_B05_SQL_001"
COMMON = previous.previous.previous.common
...
def load_batch() -> dict[str, Any]:
    ...
    checks = (
        ...
        batch.get("training_batch_id") == "level2-f5-batch06-20260716-01",
        batch.get("training_level") == EXPECTED_TRAINING_LEVEL,
        batch.get("status") == "frozen",
        batch.get("source") == "F5 Batch 06-S1定向范围确认真实验证结果",
        ...
        sample.get("expected_behavior") == "返回最多50条水安全溯源分区编码、名称和划分依据",
    )
    if not all(checks):
...
def validate_candidate_source(batch: dict[str, Any]) -> dict[str, Any]:
    names = (
        ...
        "business-object-comparison.json", "batch06-scope-summary.json",
    )
    source = {name: json.loads((SOURCE_EVIDENCE / name).read_text(encoding="utf-8")) for name in names}
    summary = source["batch06-scope-summary.json"]
    ...
    retrieval = source["retrieval-collision-analysis.json"]
    business = source["business-object-comparison.json"]
    covered_tables = {
        table for record in inventory.get("records", [])
        for table in record.get("sql_guard_used_tables", [])
    }
    checks = {
        "recommendation_equal": summary.get("batch06_scope_recommendation") == EXPECTED_CANDIDATE_ID,
        ...
        "source_valid": summary.get("candidate_source_valid") is True,
        "duplicate_found_false": duplicate.get("duplicate_found") is False,
        "metadata_valid": summary.get("metadata", {}).get("valid") is True,
        "divide_basis_readable": database.get("divide_basis_readable_text") is True,
        "sql_guard_pass": database.get("sql_guard_pass") is True,
        "database_success": database.get("database_query_success") is True,
        "result_row_count_50": database.get("result_row_count") == 50,
        "semantic_risk_low": summary.get("semantic_risk") == "LOW",
        "retrieval_collision_medium": retrieval.get("retrieval_collision_risk") == "MEDIUM",
        "business_object_distinct": business.get("business_object_distinct") is True,
        ...
        "controlled_count_5": inventory.get("controlled_tool_memory_count") == 5,
        "tool_total_69": inventory.get("total_tool_memory_count") == 69,
...
def validate_metadata() -> dict[str, Any]:
    ...
    checks = {
        "table_comment": bool(table_rows) and table_rows[0].get("table_comment") == "水安全溯源模块-三级分区表，用于管理国控断面控制单元、自动站分区、工业园区/分水岭分区",
        "zone_code_comment": fields.get("zone_code", {}).get("comment") == "分区编码",
        "zone_name_comment": fields.get("zone_name", {}).get("comment") == "分区名称",
        "divide_basis_comment": fields.get("divide_basis", {}).get("comment") == "分区划分依据，例如国控断面、自动站、工业园区、分水岭、排水片区",
        "zone_code_type": fields.get("zone_code", {}).get("type") == "character varying(100)",
        "zone_name_type": fields.get("zone_name", {}).get("type") == "character varying(200)",
        "divide_basis_type": fields.get("divide_basis", {}).get("type") == "character varying(100)",
        "divide_basis_readable": "例如" in fields.get("divide_basis", {}).get("comment", ""),
        "single_table_semantics": True,
        "no_personal_sensitive_information": True,
        "forbidden_columns_absent": not any(name in normalized_sql(EXPECTED_SQL).lower().split() for name in ("geom", "metadata_json", "create_by", "update_by")),
    }
    result = {"table": EXPECTED_TABLES[0], "checks": checks, "valid": all(checks.values()), "divide_basis_readable_text": checks["divide_basis_readable"]}
    if not result["valid"]:
...
def database_validation(sql: str) -> dict[str, Any]:
    ...
    with psycopg2.connect(
        ...
        connect_timeout=10, application_name="vanna-f5-batch06-validation",
    ...
    result = {
        ...
        "zone_code_unique_count": len({row[0] for row in rows if row[0] is not None}),
        "zone_name_unique_count": len({row[1] for row in rows if row[1] is not None}),
        "divide_basis_distinct_count": len({row[2] for row in rows if row[2] is not None}),
    ...
    expected_non_null = {"zone_code": 50, "zone_name": 50, "divide_basis": 50}
    if not all((
        len(rows) == 50, columns == EXPECTED_COLUMNS, non_null == expected_non_null,
        result["zone_code_unique_count"] == 50,
        result["zone_name_unique_count"] == 46,
        result["divide_basis_distinct_count"] == 3,
    )):
...
def _query_retrieval(memory: Any, question: str, target_sample_id: str) -> dict[str, Any]:
    ...
    context = ToolContext(
        user=User(id="f5-batch06", username="f5-batch06", group_memberships=[]),
        conversation_id="f5-batch06-bidirectional", request_id="f5-batch06-bidirectional",
...
def bidirectional_retrieval(memory: Any) -> dict[str, Any]:
    b06_query = _query_retrieval(memory, EXPECTED_QUESTION, EXPECTED_SAMPLE_ID)
    b04_query = _query_retrieval(memory, BATCH04_QUESTION, BATCH04_SAMPLE_ID)
    b06_b04 = next((item for item in b06_query["top5"] if item["sample_id"] == BATCH04_SAMPLE_ID), None)
    b06_b05 = next((item for item in b06_query["top5"] if item["sample_id"] == BATCH05_SAMPLE_ID), None)
    b04_b06 = next((item for item in b04_query["top5"] if item["sample_id"] == EXPECTED_SAMPLE_ID), None)
    checks = {
        "b06_in_top5": b06_query["target_in_top5"],
        "b06_rank_1": b06_query["target_rank"] == 1,
        "b06_injected": b06_query["target_injected"],
        "b06_not_filtered": not b06_query["target_filtered"],
        "b06_sql_exact": normalized_sql(b06_query["target_injected_sql"]) == normalized_sql(EXPECTED_SQL),
        "b04_below_b06_for_b06_query": b06_b04 is None or b06_b04["rank"] > 1,
        "b04_in_top5": b04_query["target_in_top5"],
        "b04_rank_1": b04_query["target_rank"] == 1,
        "b04_injected": b04_query["target_injected"],
        "b04_not_filtered": not b04_query["target_filtered"],
        "b04_sql_exact": normalized_sql(b04_query["target_injected_sql"]) == normalized_sql(BATCH04_SQL),
        "b06_below_b04_for_b04_query": b04_b06 is None or b04_b06["rank"] > b04_query["target_rank"],
    }
    result = {
        "b06_query": b06_query, "b04_query": b04_query,
        "b06_query_b04_memory_rank": b06_b04["rank"] if b06_b04 else None,
        "b06_query_b05_memory_rank": b06_b05["rank"] if b06_b05 else None,
        "b04_query_b06_memory_rank": b04_b06["rank"] if b04_b06 else None,
...
def run_regressions(data_dir: Path, agent_dir: Path) -> tuple[dict[str, Any], list[str], str]:
    ...
    try:
        ...
        b5_case = run_case({"id": "B05_TARGET", "query": BATCH05_QUESTION, "tables": BATCH05_TABLES, "limit": 50}, agent_dir, True)
        b6_case = run_case({"id": "B06_TARGET", "query": EXPECTED_QUESTION, "tables": EXPECTED_TABLES, "limit": 50}, agent_dir, True)
        ...
        b5 = validate_target(b5_case, question=BATCH05_QUESTION, tables=BATCH05_TABLES, columns=BATCH05_COLUMNS, expected_rows=12)
        b6 = validate_target(b6_case, question=EXPECTED_QUESTION, tables=EXPECTED_TABLES, columns=EXPECTED_COLUMNS, expected_rows=50)
        validations = (b2, b3, b4, b5, b6)
        pass_count = f2["question_pass_count"] + sum(int(item["accepted"]) for item in validations)
        return ({
            "question_count": 11, "question_pass_count": pass_count,
            "accepted": COMMON.regression_passed(f2) and all(item["accepted"] for item in validations),
            ...
            "batch06_case": b6_case, "batch06_validation": b6,
        }, logs, key)
    finally:
        stop_server(process)
...
def run_isolated_worker(root: Path, data: Path, prefix: str) -> int:
    ...
    try:
        ...
        preflight, write_result, retrieval = write_and_verify(memory, adapter, plan, 192)
        ...
        accepted = regression["accepted"] and retrieval["accepted"] and sqlite_record_count(data) == 193
        summary = {
            "accepted": accepted, "initial_count": 192,
...
def retryable_failure(summary: dict[str, Any]) -> tuple[bool, str]:
    ...
    if regression.get("question_count") != 11 or regression.get("question_pass_count") != 10:
        return False, "NOT_SINGLE_QUESTION_FAILURE"
    failed_f2 = [item for item in regression.get("f2_cases", []) if not item.get("passed")]
    targets = [regression.get(name, {}) for name in (
        "batch02_validation", "batch03_validation", "batch04_validation",
        "batch05_validation", "batch06_validation",
    )]
...
def main() -> int:
    ...
    if sqlite_record_count(RUNTIME_SOURCE) != 192 or runtime_before["content_sha256"] != EXPECTED_FORMAL_SHA256:
    ...
    root = BACKUP_PARENT / f"f5-level2-batch06-{timestamp}"
    evidence = root / "evidence"
    backup_data = BACKUP_PARENT / f"f5-level2-batch06-prewrite-{timestamp}" / "runtime-vanna_data"
    ...
    backup_payload = {
        ...
        "verified": backup.destination.content_sha256 == EXPECTED_FORMAL_SHA256 and sqlite_record_count(backup_data) == 192,
    }
    ...
    if accepted_prefix is None:
        summary = {
            "f5_batch06_accepted": False, "failure_stage": "isolated", "attempts": attempts,
            "worker_output": output, "formal_opened": False,
            "formal_write_executed": False, "recovery_executed": False,
        }
        write_json(evidence / "f5-batch06-summary.json", sanitized(summary))
    ...
    if sqlite_record_count(RUNTIME_SOURCE) != 192 or current["content_sha256"] != EXPECTED_FORMAL_SHA256:
    ...
    try:
        ...
        if not regression["accepted"] or not retrieval["accepted"] or after_smoke != 193:
            raise RuntimeError("FORMAL_ACCEPTANCE_FAILED")
        summary = {
            "f5_batch06_accepted": True, "root": str(root), "backup_path": str(backup_data),
