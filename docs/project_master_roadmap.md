...
```text
B4 已完成：固定数据源水质日报/月报、确定性计算、页面预览、PDF 导出、
真实历史数据验收和对话一句话生成均已实现。
```
...
F6-1I-B-R2 完整隔离候选与切换/回滚演练 ✅ 已完成并验收
F6-1I-C-A 切换后验收与回滚证明 ✅ 已完成
F6-1I-C 首次正式执行：ROLLBACK_VERIFICATION_FAILED，现场冻结
F6-1I-C-R1 正式监控和目录句柄隔离修复 ✅ 已完成
F6-1I-C-R2 只读事故诊断 ✅ 已完成，current live 为完整 managed 新库
F6-1I-C-R3-A 独立保留验收接口 ✅ 已完成
F6-1I-C-R3-B 独立正式验收 ✅ CURRENT_LIVE_FORMALLY_ACCEPTED
current live：正式保留
pre-switch：作为旧基线备份保留，不再恢复
F6-1 正式收口：✅ 已完成
F6-2 Metadata 更新机制：未开始
```
...
R2 收口时，F6-1A～H、F6-1I-A/R1、F6-1I-B-R1/R2 已完成，I-B 首次执行安全停止，F6-1I-B-R2 等待审查，F6-1I-C 尚未开始；后续状态由 12.19 继续记录。

### 12.19 F6-1I-C-A 正式切换后验收与回滚证明（2026-07-21）

正式接口新增原始 I-B summary 固定 SHA 参数。执行顺序冻结为 summary 字节 SHA、I-B 全部关键事实、三个运行时确认、路径参数、正式来源访问；前四层任一失败时正式路径文件系统访问和 Client 创建均为 0。获批 summary 路径和 SHA 已固定在治理 SOP 中，原始文件不得修改。

切换前必须独立完成 candidate 分类/Plan/Top-K、15/15 完整回归和同级 candidate Tree SHA 对账。切换后必须重新执行新正式库分类/Plan/重复验收、旧来源与新正式查询副本的 12 题 Top-K 语义验收，以及从新正式库全新副本执行的第二次 15/15 完整回归；PASS summary 分别记录切换前和切换后结果。

第二步重命名、切换后分类、Top-K 或完整回归失败均自动回滚。恢复后必须验证 Tree SHA，并从恢复库只读副本验证总数、分类和治理事实；验证失败固定为 `ROLLBACK_VERIFICATION_FAILED`，不得宣称恢复。成功路径保留 pre-switch 且不主动回滚。

合成自检覆盖所有新增门禁，正式 Chroma 文件系统访问 0、真实 Client 创建 0、正式切换 0。Evidence：`E:\3\_training_backups\f6-1i-c-a-20260721-093308\evidence`。

I-C-A 收口时，F6-1A～H、F6-1I-A/R1、F6-1I-B-R1/R2 已完成并验收，F6-1I-C-A 已准备等待审查，F6-1I-C 正式执行尚未开始；后续状态由 12.20 继续记录。

### 12.20 F6-1I-C-R1 正式监控基线、目录句柄隔离与事故诊断准备（2026-07-21）

F6-1I-C 首次正式执行在切换前候选、Top-K、15/15 回归以及切换后分类和 Top-K 均通过后，因 Runner 继续使用旧正式固定 SHA，在切换后 `before_service_start` 误报 `FormalRuntimeChanged`；随后直接打开过的新正式路径在自动回滚移动时发生 `WinError 5`，最终为 `ROLLBACK_VERIFICATION_FAILED`。事故现场冻结，本阶段未读取或操作 current live、pre-switch、failed 目录或失败运行目录。

Runner 已新增成对动态正式监控参数，保留旧命令默认基线，所有 checkpoint 继续执行 count/SHA fail-fast。Governance 以相同 `directory_state()` 算法分别冻结切换前旧正式和切换后新正式基线。切换后分类、Top-K、完整回归和回滚分类全部改为只打开独立副本；任何 Client 打开路径与同级重命名目标相交时，以 `CLIENT_RENAME_TARGET_CONFLICT` 在重命名前停止。

未来 `--incident-diagnose` 接口只读复制并打开两个仓库外副本，不移动事故目录，输出 managed-current/legacy-pre、invalid-current/recoverable-pre 或 blocked 三种状态。诊断、保留 current live、受控回滚分别需要新授权。合成 Evidence：`E:\3\_training_backups\f6-1i-c-r1-20260721-101126\evidence`。

F6-1A～H、F6-1I-A/R1、I-B-R1/R2、I-C-A 已完成；I-C 首次正式执行为 `ROLLBACK_VERIFICATION_FAILED`；I-C-R1 已完成。后续诊断与验收准备状态由 12.21 继续记录。

### 12.21 F6-1I-C-R2 诊断与 R3 独立保留验收收口（2026-07-21）

R2 只读诊断确认 `CURRENT_LIVE_MANAGED_PRE_SWITCH_LEGACY`：current live 为 `198 total / 115 managed v1 / 0 legacy / 83 non-DDL`、Plan `0/115/0/0` 的完整新库；pre-switch 为 `198 total / 0 managed v1 / 115 legacy / 83 non-DDL` 的完整旧基线。两侧非 DDL 三元签名一致，首次失败 Evidence 对账通过，诊断未直接打开正式 Client、未重命名、未恢复。

R3-A 已准备未来接口 `--incident-accept-current-live`。它先验证两份 Evidence 的固定 SHA 和关键事实，再验证保留授权、服务停止、无 Client 占用和固定路径，最后才允许访问 current live/pre-switch。验收只使用分类、Top-K 和回归副本，要求 current live 分类与 Plan、pre-switch legacy 基线、83 条非 DDL、固定 Top-K、15/15 完整回归以及验收后来源完整性全部通过；不执行切换、恢复、重命名、collection 写入或 pre-switch 删除。

R3-A 合成自检覆盖前置零访问、只开验收副本、Plan `0/115/0/0`、非 DDL 83 条、Top-K `12/10/3/9/10`、15/15、来源变化失败和成功状态 `CURRENT_LIVE_FORMALLY_ACCEPTED`。事故运行时文件系统访问 0、真实 Client 0、正式验收 0、切换 0、恢复 0。Evidence：`E:\3\_training_backups\f6-1i-c-r3-a-20260721-104718\evidence`。

R3-B 在 `E:\3\_training_backups\f6-1i-c-r3-20260721-105855` 完成独立正式验收，状态为 `CURRENT_LIVE_FORMALLY_ACCEPTED`。current live 为 `198 total / 115 managed v1 / 0 legacy / 83 non-DDL`，Plan `0/115/0/0`；pre-switch 为 `198 total / 0 managed / 115 legacy / 83 non-DDL`。非 DDL 保真、固定 Top-K 语义 SHA 与 `3/9/10` 命中、15/15 完整回归和验收后来源完整性全部通过；collection 写入为 0，未执行正式切换、重命名或恢复。

最终保留 current live；pre-switch 继续作为旧基线备份保留，不再恢复。首次正式执行的 `ROLLBACK_VERIFICATION_FAILED` 继续作为历史事故事实记录。F6-1 DDL Text Memory 幂等治理已正式完成。

---

## 13. F6-2：Metadata 更新机制

F6-2A PostgreSQL Metadata 只读刷新与差异报告已完成：
...
B3 本身不包含日报月报；该能力已在后续独立 B4 板块完成。
...
| 日报月报 | 已完成 | B4：确定性日报/月报、确认优先配置面板、不可变预览与 PDF |
...
B4 使用 `mysql-lzh-monitor` 固定只读数据源，实现确定性日报/月报计算、A4 PDF、应用内预览和对话一句话生成。报表只从对话进入，不在侧栏设置独立页面。报表计算不依赖 LLM；`wm_section_wq_info` 仅作为报表专用水质目标白名单，不进入 B3 Metadata 或 Chroma。

## 37.1 B4 对话预览正式收口

- 所有聊天报表生成意图统一只返回 `report_config`；即使周期、指标、频次和回看范围完整，也必须由用户确认后才能调用生成接口。
- 普通查询继续进入通用 Agent；明确日报/月报锁定类型，泛化“生成报表”默认日报并允许用户切换日报/月报。
- `report_config` 不进入消息历史，按会话单独持久化；配置面板固定在输入框上方，支持刷新恢复、会话隔离、取消和失败后保留参数。
- 日报选择一个日期和近 2—7 日回看范围；月报只选择一个月份，不显示回看天数，但与日报共用真实指标多选和真实频次选择。
- `POST /api/reports/water-quality/generate` 只计算一次并生成不可变 `report_id` 快照；预览与 PDF 均按同一 `report_id` 读取，避免不同指标、频次或回看范围相互覆盖。
- 结果卡支持基于旧快照参数读取最新真实选项并重新生成；旧结果与新 `report_id` 同时保留。
- 主工作台和 Widget 共用紧凑配置面板；Widget 仍受 `allowed_source_ids` 和数据源可用状态约束，不显示内部 `source_id`。
- 旧历史 `report_config` 消息卡继续只读兼容；新配置不再作为消息写入。
...
B5 已将静态双数据源注册升级为 SQLite 动态目录。默认目录为 `agent_data/data_sources/catalog.sqlite3`，支持 `DATA_SOURCE_CATALOG_PATH` 覆盖，并以 schema version 5 事务迁移现有 `postgresql-main` 和 `mysql-lzh-monitor`；v2 增加 MySQL TLS 模式及证书路径，v3 增加幂等 pending cleanup，v4 增加 active batch，v5 增加完整发布快照、资产计划、逐项进度、owner 和恢复错误状态。
...
- 生命周期统一为 `draft / connected / metadata_ready / training_required / ready / disabled / error`，启停只允许 `ready → disabled` 与已完成正式发布的 `disabled → ready`；其他状态由目录层拒绝。
...
- Runtime 缓存绑定 `runtime_revision`，请求以租约持有旧 Runtime；prepare 不预先 invalidate，只有新 Runtime 构建和验证成功后才原子切换缓存。构建失败时旧 Runtime/revision、旧 Memory 和五类正式资产完整保留，活跃请求及后续 require 继续使用旧实例；关闭失败不会触发 Memory 清理。
...
- 发布对 Metadata、revision Memory、DDL、业务文档、asset manifest 和 Catalog 状态执行状态机协调；阶段为 `prepared / backing_up / installing / catalog_published / runtime_validated / committed / rolling_back / rollback_failed`。每项移动后持久化进度，任意进程中断后只能恢复为完整旧版或完整新版。
- manifest 绑定 source、revision、scope fingerprint、Memory identity 和四类内容哈希。启动按 owner、阶段、Catalog 与实际哈希前滚或回滚；恢复中再次崩溃可幂等继续。无法证明一致时保留 batch/backup 并关闭该源问数。
- 清理器只处理动态源自己的受管根目录并保护 Catalog 当前五类正式资产、当前/retired/关闭失败 Runtime 和活动批次路径；恢复时五类正式、候选和 backup 路径必须与批次可推导值完全一致，owner 存活时不接管。Runtime 释放回调只重试明确 pending，不扫描候选或 backup。内置 B3、B4、其他源和目录外资产不清理。
- 候选 Chroma 只释放自身 executor，不停止 SharedSystem 或清空全局 cache；关闭一个候选或数据源 Runtime 不影响其他源。
