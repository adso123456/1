...
```text
B4 已完成；下一动作按业务优先级进入外部网站机器人或生产化治理。
```
...
- 报表意图按“普通问数 / 周期待补充 / 周期明确”三级处理；周期待补充时只返回 `report_config`，不查询报表数据或生成 PDF。
- 周期明确时返回 `report_result`。日报选择一个日期和近 2—7 日回看范围；月报只选择一个月份，不显示回看天数，但与日报共用指标多选和真实频次选择。
