...
// App 和 Widget 都在新会话按服务端会话绑定拉取推荐问题。
...
const checks = [
  ...
  ['ChatArea 按 prop 渲染推荐问题', chatArea.includes('suggestions.map(item =>') && chatArea.includes('onSend(item.text, item.id)')],
  ...
  ['Widget 通过 RPC 绑定会话并拉取推荐问题', widget.includes("'bind-conversation-source'") && widget.includes("'suggested-questions'")],
  ['拉取地址只含会话 ID，不含可伪造的 source_id', app.includes('/api/conversations/') && !app.includes('suggested-questions?source_id=') && !widget.includes('suggested-questions?source_id=')],
];
