...
const composer = read('src/components/ReportComposerPanel.tsx');
...
const assertions = [
  ['快照生成 API', composer.includes('/api/reports/water-quality/generate')],
  ...
  ['日报2至7日', composer.includes('config.available_recent_days.map')],
  ['指标多选', composer.includes('type="checkbox"')],
  ['真实频次选项', composer.includes('按站点配置')],
  ['聊天共享参数组件', components.includes('<ReportParameterForm')],
  ['聊天配置卡', messageBubble.includes('<ReportConfigCard')],
  ['输入框上方配置面板', composer.includes('ReportComposerPanel')],
