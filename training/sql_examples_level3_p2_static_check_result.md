...
| L3_P2_SQL_003 | 查询排污口国家编码、名称及对应实况记录明细中的排水特征、在线监测和采样条件状态 | rs_outlet_info_v2, rs_outlet_live_v2 | INNER JOIN | rs_outlet_info_v2.id=rs_outlet_live_v2.outlet_id | metadata 注释：主键ID关联实况表的排污口ID；两侧均为 bigint。 | rs_outlet_info_v2, rs_outlet_live_v2 | 无 | 无 | 无 | passed=True, severity=ok | pass |
...
| L3_P2_SQL_005 | 查询各断面每年度的全年水质目标等级记录 | wm_section_info, wm_section_wq_info | INNER JOIN | wm_section_info.id=wm_section_wq_info.section_id | metadata 两侧注释均为断面id，类型均为 bigint；month=0 的注释明确表示全年。 | wm_section_info, wm_section_wq_info | 无 | 无 | 无 | passed=True, severity=ok | pass |
