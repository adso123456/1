...
def self_test(suite_path: Path, evidence_dir: Path | None) -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="request-trace-self-test-") as trace_name:
        ...
        initialize_request_diagnostics("deepseek policy")
        initialize_run_sql_requirement()
        record_injected_sql_examples(1)
        deep_request_before = fake_request.model_dump(mode="python")

        async def run_deepseek_two_calls() -> None:
            await deep_service.send_request(fake_request)
            await deep_service.send_request(fake_request)

        asyncio.run(run_deepseek_two_calls())
        deep_first_payload, deep_second_payload = deep_service.parent_payloads
        ...
        deepseek_second_call_thinking_restored_test = (
            deep_second_payload.get("extra_body", {}).get("thinking")
            == {"type": "enabled"}
        )
        deepseek_second_call_auto_restored_test = (
        ...
        non_streaming_policy_test = (
            deep_service.parent_call_count == 2
        ...
        streaming_policy_test = (
            stream_policy_service.parent_call_count == 1
        ...
        async def run_provider_exception() -> tuple[bool, bool, bool, bool]:
            ...
            return propagated, request_end_valid, cleaned, double_safe
        ...
        no_provider_retry_on_bad_request_test = (
            exception_propagated and failing_service.parent_call_count == 1
        )
    ...
    payload = {
        ...
        "DEEPSEEK_THINKING_FIRST_CALL_DISABLED_TEST": "PASS" if deepseek_thinking_first_call_disabled_test else "FAIL",
        "DEEPSEEK_FIRST_CALL_NAMED_RUN_SQL_TEST": "PASS" if deepseek_first_call_named_run_sql_test else "FAIL",
        "DEEPSEEK_FIRST_CALL_REASONING_EFFORT_REMOVED_TEST": "PASS" if deepseek_first_call_reasoning_effort_removed_test else "FAIL",
        "DEEPSEEK_SECOND_CALL_THINKING_RESTORED_TEST": "PASS" if deepseek_second_call_thinking_restored_test else "FAIL",
        "DEEPSEEK_SECOND_CALL_AUTO_RESTORED_TEST": "PASS" if deepseek_second_call_auto_restored_test else "FAIL",
        "DEEPSEEK_EXTRA_BODY_FIELDS_PRESERVED_TEST": "PASS" if deepseek_extra_body_fields_preserved_test else "FAIL",
        ...
        "NO_PROVIDER_RETRY_ON_BAD_REQUEST_TEST": "PASS" if no_provider_retry_on_bad_request_test else "FAIL",
        "STREAMING_POLICY_TEST": "PASS" if streaming_policy_test else "FAIL",
        "NON_STREAMING_POLICY_TEST": "PASS" if non_streaming_policy_test else "FAIL",
    ...
    if evidence_dir:
        ...
        write_json(
            evidence_dir / "provider-policy-test-results.json",
            {
                key: payload[key]
                for key in (
                    "DEEPSEEK_THINKING_FIRST_CALL_DISABLED_TEST",
        ...
        write_json(
            evidence_dir / "exception-cleanup-test-results.json",
            {
                key: payload[key]
                for key in (
                    "PROVIDER_EXCEPTION_REQUEST_END_WRITTEN_TEST",
